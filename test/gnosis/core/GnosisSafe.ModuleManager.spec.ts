...
describe("ModuleManager", async () => {
  ...
  describe("enableModule", async () => {
    it("can only be called from Safe itself", async () => {
      ...
      await expect(safe.enableModule(user2.address)).to.be.revertedWith(
        "GS031"
      );
    });
    ...
    it("can enable multiple", async () => {
      ...
      await expect(
        await safe.getModulesPaginated(AddressOne, 10)
      ).to.be.deep.equal([[user2.address, user1.address], AddressOne]);
    });
  });
  ...
  describe("disableModule", async () => {
    it("can only be called from Safe itself", async () => {
      const { safe } = await setupTests();
      await expect(
        safe.disableModule(AddressOne, user2.address)
  ...
  describe("execTransactionFromModuleReturnData", async () => {
    ...
    it("Returns expected from contract on failure", async () => {
      ...
      await mock.givenCalldataRevertWithMessage(
        "0xbaddad",
        "Some random message"
      );
      await expect(
