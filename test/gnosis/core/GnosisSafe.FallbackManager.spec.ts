...
describe("FallbackManager", async () => {
  const setupWithTemplate = deployments.createFixture(
    async ({ deployments }) => {
  ...
  describe("setFallbackManager", async () => {
    it("is correctly set on deployment", async () => {
      ...
      await expect(
        ...
        "0x" + handler.address.toLowerCase().slice(2).padStart(64, "0")
      );
    });
    ...
    it("is called when set", async () => {
      const { safe } = await setupWithTemplate();
      const handler = await defaultCallbackHandlerDeployment();
      const safeHandler = (await defaultCallbackHandlerContract()).attach(
        safe.address
      );
      ...
      await expect(
        safeHandler.callStatic.onERC1155Received(
          AddressZero,
          AddressZero,
          0,
          0,
