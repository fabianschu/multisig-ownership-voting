...
describe("GnosisSafe", async () => {
  ...
  describe("setup", async () => {
    it("should not allow to call setup on singleton", async () => {
      ...
      await expect(await singleton.getThreshold()).to.be.deep.eq(
        BigNumber.from(1)
      );
      await expect(
    ...
    it("should set domain hash", async () => {
      ...
      await expect(await template.getThreshold()).to.be.deep.eq(
        BigNumber.from(2)
      );
    });
    ...
    it("should work with ether payment to deployer", async () => {
      ...
      await expect(
        await hre.ethers.provider.getBalance(template.address)
      ).to.be.deep.eq(parseEther("10"));
      ...
      await expect(
        userBalance.lt(await hre.ethers.provider.getBalance(user1.address))
      ).to.be.true;
    });
    ...
    it("should work with ether payment to account", async () => {
      ...
      ).to.be.deep.eq(userBalance.add(payment));
