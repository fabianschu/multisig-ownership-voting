...
describe("Upgrade from Safe 1.1.1", () => {
  ...
  const ChangeMasterCopyInterface = new ethers.utils.Interface([
    "function changeMasterCopy(address target)",
  ]);
  ...
  const setupTests = deployments.createFixture(async ({ deployments }) => {
    ...
    const singleton111 = (
      await (
        await user1.sendTransaction({ data: deploymentData.safe111 })
      ).wait()
    ).contractAddress;
    ...
    await factory
      .createProxyWithNonce(singleton111, "0x", saltNonce)
      .then((tx: any) => tx.wait());
    ...
    const data = ChangeMasterCopyInterface.encodeFunctionData(
      "changeMasterCopy",
      [singleton130]
    );
