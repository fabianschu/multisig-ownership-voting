...
export const verificationTests = (setupTests: () => Promise<TestSetup>) => {
  ...
  describe("execTransaction", async () => {
    it("should be able to transfer ETH", async () => {
      ...
      await expect(
        await ethers.provider.getBalance(migratedSafe.address)
  ...
  describe("addOwner", async () => {
    it("should add owner and change treshold", async () => {
      ...
      await expect(await migratedSafe.getThreshold()).to.be.deep.eq(
        BigNumber.from(2)
      );
      await expect(await migratedSafe.getOwners()).to.be.deep.equal([
        user2.address,
        user1.address,
      ]);
  ...
  describe("enableModule", async () => {
    it("should enabled module and be able to use it", async () => {
      ...
      await expect(await migratedSafe.isModuleEnabled(user2.address)).to.be
        .true;
      await expect(
        await migratedSafe.getModulesPaginated(AddressOne, 10)
  ...
  describe("multiSend", async () => {
    it("execute multisend via delegatecall", async () => {
      ...
      await expect(
        await hre.ethers.provider.getBalance(migratedSafe.address)
      ).to.be.deep.eq(parseEther("1"));
      ...
      const safeTx = buildMultiSendSafeTx(
        multiSend,
        txs,
        await migratedSafe.nonce()
      );
      await expect(executeTxWithSigners(migratedSafe, safeTx, [user1])).to.emit(
        migratedSafe,
        "ExecutionSuccess"
      );
      ...
      await expect(
        await hre.ethers.provider.getBalance(user2.address)
      ).to.be.deep.eq(userBalance.add(parseEther("1")));
      expect(
        await mock.callStatic.invocationCountForCalldata("0xbaddad")
      ).to.be.deep.equals(BigNumber.from(1));
