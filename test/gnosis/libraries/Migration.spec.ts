...
describe("Migration", async () => {
  ...
  const setupTests = deployments.createFixture(async ({ deployments }) => {
    ...
    const singleton120 = (
      await (
        await user1.sendTransaction({ data: deploymentData.safe120 })
      ).wait()
    ).contractAddress;
  ...
  describe("constructor", async () => {
    it("can not use 0 Address", async () => {
      ...
      await expect(user1.sendTransaction(tx)).to.be.revertedWith(
  ...
  describe("migrate", async () => {
    ...
    it("can migrate", async () => {
      ...
      const expectedDomainSeparator = ethers.utils._TypedDataEncoder.hashDomain(
      ...
      const respData = await user1.call({
        to: safe.address,
        data: MigratedInterface.encodeFunctionData("domainSeparator"),
      });
      await expect(
        MigratedInterface.decodeFunctionResult("domainSeparator", respData)[0]
      ).to.be.eq(expectedDomainSeparator);

      const masterCopyResp = await user1.call({
        to: safe.address,
        data: MigratedInterface.encodeFunctionData("masterCopy"),
      });
      await expect(
        MigratedInterface.decodeFunctionResult("masterCopy", masterCopyResp)[0]
      ).to.be.eq(singleton120);
    });
  });
});
