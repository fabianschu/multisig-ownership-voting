...
describe("CreateCall", async () => {
  ...
  describe("performCreate", async () => {
    ...
    it("can call factory directly", async () => {
      const { createCall, testContract } = await setupTests();
      const createCallNonce = await ethers.provider.getTransactionCount(
        createCall.address
      );
      const address = ethers.utils.getContractAddress({
        from: createCall.address,
        nonce: createCallNonce,
      });

      await expect(createCall.performCreate(0, testContract.data))
    ...
    it("should successfully create contract and emit event", async () => {
      ...
      const safeEthereumNonce = await ethers.provider.getTransactionCount(
        safe.address
      );
      const address = ethers.utils.getContractAddress({
        from: safe.address,
        nonce: safeEthereumNonce,
      });
    ...
    it("should successfully create contract and send along ether", async () => {
      ...
      const tx = await buildContractCall(
        createCall,
        "performCreate",
        [parseEther("1"), testContract.data],
        await safe.nonce(),
        true
      );
      await expect(
      ...
      await expect(await hre.ethers.provider.getBalance(address)).to.be.deep.eq(
        parseEther("1")
      );
      const newContract = new ethers.Contract(
        address,
  ...
  describe("performCreate2", async () => {
    ...
    it("can call factory directly", async () => {
      ...
      const address = ethers.utils.getCreate2Address(
        createCall.address,
        salt,
        ethers.utils.keccak256(testContract.data)
      );
