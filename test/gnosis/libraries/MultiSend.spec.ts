...
describe("MultiSend", async () => {
  ...
  describe("multiSend", async () => {
    it("should enforce delegatecall to MultiSend", async () => {
      ...
      const nestedTransactionData = encodeMultiSend([
        buildContractCall(killLib, "killme", [], 0),
      ]);
      ...
      expect(await hre.ethers.provider.getCode(multiSend.address)).to.be.eq(
        multiSendCode
      );
    });
    ...
    it("Should fail when using invalid operation", async () => {
      ...
      const txs = [
        buildSafeTransaction({ to: user2.address, operation: 2, nonce: 0 }),
      ];
