...
describe("CompatibilityFallbackHandler", async () => {
  ...
  const setupTests = deployments.createFixture(async ({ deployments }) => {
    ...
    const validator = (await compatFallbackHandlerContract()).attach(
      safe.address
    );
  ...
  describe("isValidSignature(bytes,bytes)", async () => {
    ...
    it("should revert if message was not signed", async () => {
      ...
      await expect(
        validator.callStatic["isValidSignature(bytes,bytes)"]("0xbaddad", "0x")
    ...
    it("should revert if signature is not valid", async () => {
      const { validator } = await setupTests();
      await expect(
        validator.callStatic["isValidSignature(bytes,bytes)"](
          "0xbaddad",
          "0xdeaddeaddeaddead"
        )
      ).to.be.reverted;
    ...
    it("should return magic value if message was signed", async () => {
      ...
      expect(
        await validator.callStatic["isValidSignature(bytes,bytes)"](
          "0xbaddad",
          "0x"
        )
      ).to.be.eq("0x20c13b0b");
  ...
  describe("isValidSignature(bytes32,bytes)", async () => {
    ...
    it("should revert if message was not signed", async () => {
      ...
      const dataHash = ethers.utils.keccak256("0xbaddad");
      await expect(
        validator.callStatic["isValidSignature(bytes32,bytes)"](dataHash, "0x")
      ).to.be.revertedWith("Hash not approved");
    });
    ...
    it("should revert if signature is not valid", async () => {
      ...
      const dataHash = ethers.utils.keccak256("0xbaddad");
      await expect(
        validator.callStatic["isValidSignature(bytes32,bytes)"](
          dataHash,
          "0xdeaddeaddeaddead"
        )
      ).to.be.reverted;
  ...
  describe("getModules", async () => {
    it("returns enabled modules", async () => {
      ...
      await expect(await validator.getModules()).to.be.deep.equal([
        user2.address,
      ]);
    });
  });
  ...
  describe("simulate", async () => {
    ...
    it("should revert changes", async () => {
      ...
      expect(
        await validator.callStatic.simulate(
          killLib.address,
    ...
    it("should return result", async () => {
      ...
      expect(
        await validator.callStatic.simulate(
          killLib.address,
          killLib.interface.encodeFunctionData("expose")
        )
      ).to.be.eq(
    ...
    it("should simulate transaction", async () => {
      ...
      const estimate = await validator.callStatic.simulate(
        killLib.address,
        killLib.interface.encodeFunctionData("estimate", [
          validator.address,
          "0x",
        ])
      );
