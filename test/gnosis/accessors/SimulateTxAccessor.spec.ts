...
describe("SimulateTxAccessor", async () => {
  ...
  describe("estimate", async () => {
    it("should enforce delegatecall", async () => {
      ...
      expect(await hre.ethers.provider.getCode(accessor.address)).to.be.eq(
        code
      );
    });
    ...
    it("simulate call", async () => {
      ...
      const simulationData = accessor.interface.encodeFunctionData("simulate", [
        tx.to,
        tx.value,
        tx.data,
        tx.operation,
      ]);
      const acccessibleData = await simulator.callStatic.simulate(
        accessor.address,
        simulationData
      );
      const simulation = accessor.interface.decodeFunctionResult(
        "simulate",
        acccessibleData
      );
      expect(
        safe.interface.decodeFunctionResult(
          "getOwners",
    ...
    it("simulate delegatecall", async () => {
      ...
      expect(
        interactor.interface.decodeFunctionResult(
          "sendAndReturnBalance",
          simulation.returnData
        )[0]
